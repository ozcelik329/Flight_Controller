...
void SensorFusion::update(float gx, float gy, float gz,
    ...
    s0 = -J_14or21 * f2 + J_11or24 * f1 - J_41 * f4 - J_51 * f5 - J_61 * f6;
    s1 = J_13or22 * f1 + J_12or23 * f2 - J_42 * f4 + J_52 * f5 + J_62 * f6;
    s2 = J_13or22 * f2 - J_33 * f3 - J_43 * f4 + J_53 * f5 + J_63 * f6;
    s3 = J_12or23 * f1 + J_11or24 * f2 - J_44 * f4 - J_54 * f5 - J_64 * f6;
...
void SensorFusion::updateIMU(float gx, float gy, float gz,
                             float ax, float ay, float az) {
    uint32_t now = micros();
    float dt = (now - lastUpdate) / 1000000.0f;
    if (dt <= 0.0f) {
...
float SensorFusion::getRoll() const {
    return roll;
}

float SensorFusion::getPitch() const {
    return pitch;
}

float SensorFusion::getYaw() const {
    return yaw;
}

void SensorFusion::computeAngles() {
